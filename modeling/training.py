...
def expected_patches_per_tile(h, w, patch_size, stride):
    """
    Computes expected patch grid counts (n_h, n_w) and total patches n_total, using the same
    inclusive stepping logic as extract_patches (r<=h-patch_size, c<=w-patch_size).
...
def main_training():
    ...
    train_patches, train_tile_patch_index = build_training_patches(
        train_tiles=train_tiles, patch_size=patch_size, stride=stride
    )
    teseachi_patches = build_teseachi_patches(
        teseachi_path=teseachi_path, patch_size=patch_size, stride=stride
    )
    ...
    print(
        f"We extracted {len(train_patches)} training patches total (expected {len(train_tiles)} * {per_tile_expected} = {total_expected}; "
        f"tile size {h0}x{w0} -> {n_h0}x{n_w0} patches)."
    )
    print(
        f"We extracted {len(teseachi_patches)} Teseachi patches (expected {teseachi_expected}; "
        f"tile size {hT}x{wT} -> {n_hT}x{n_wT} patches)."
    )
    ...
    ssl_loader = DataLoader(
        ssl_dataset, batch_size=batch_size, shuffle=shuffle, num_workers=num_workers
    )
